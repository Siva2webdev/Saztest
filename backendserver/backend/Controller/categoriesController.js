...
//GET by _id
const categoriesbyObId=async(req,res)=>
{
try{

  const data=await categories.findById(req.params._id)
...
const {  
  ...
  try{
    ...
    const newData=new categories({
        ...
        is_deleted })
    await newData.save();
    const videodata = await addcategories.find();
    return res.json(await addcategories.find());
...
try{
  ...
  const deletedcategories=await categories.findByIdAndDelete(req.params._id)
  if(!deletedcategories) res.status(404).send("No item found")
...
const patchId=req.params._id;
  ...
  const updated=await categories.findOneAndUpdate({_id:patchId},
   req.body,{new:true})
