...
const Playlistsaz1data = async (req, res) => {
  try {
    const data = await Playlist_saz1.find();
...
const Playlistsaz1byObId = async (req, res) => {
  try {
    const data = await Playlist_saz1.findById(req.params._id);
...
const addPlaylistsaz1 = async (req, res) => {
  ...
  try {
    const newData = new Playlist_saz1({
      id,
      title,
      status,
      image,
    ...
    await newData.save();
    const videodata = await Playlist_saz1.find();
    return res.json(await Playlist_saz1.find());
...
const deletePlaylistsaz1 = async (req, res) => {
  try {
    const deletedPlaylistsaz1 = await Playlist_saz1.findByIdAndDelete(
      req.params._id
    );
...
const Playlistsaz1Patch = async (req, res) => {
  try {
    ...
    const updated = await Playlist_saz1.findOneAndUpdate(
      { _id: patchId },
      req.body,
      { new: true }
    );
