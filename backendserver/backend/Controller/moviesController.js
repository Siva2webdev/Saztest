...
//POST API
const addMovie=asyncErrorHandler(async(req,res)=>
{
...
const {  kameraUrl,
  ...
  probe_status }= req.body;
    ...
    const newData=new videos({  kameraUrl,
      kameraUrl_ios,
      kameraUrl_android,
      kameraUrl_setup,
      kameraUrl_linux,
...
try{
  ...
  const deletedMovie=await videos.findByIdAndDelete(req.params._id)
  if(!deletedMovie) res.status(404).send("No item found")
...
const patchId=req.params._id;
  ...
  const updated=await movies.findOneAndUpdate({_id:patchId},
   req.body,{new:true})
